...
// [3] 상세 출력 함수, 실행조건 : 전체 출력화면에서 특정한 제목을 클릭했을때
function 상세출력함수(index) {
  console.log('상세출력함수 실행');
  console.log(index);
  // 1. 어디에, document.querySelector()
  // 2. 무엇을, 배열 정보를 html 로 구성, 선택한 게시물 인덱스의 정보를 , index
  let board = 게시물목록[index]; // (1) 선택한 게시물 인덱스의 게시물 호출
  let info = board.split(','); // (2) 게시물 문자열, (쉼표) 기준으로 분해
  // info[0] = 제목 , info[1] = 내용 , info[2] = 비밀번호 , info[3] = 날짜 , info[4] = 조회수
  // 3. 출력, innerHTML
  document.querySelector('.titleBox').innerHTML = info[0]; // 제목 데이터를 .titleBox 마크업 사이에 대입
  document.querySelector('.contentBox').innerHTML = info[2]; // 내용 데이터를 .contentBox 마크업 사이에 대입
  document.querySelector('.dateBox').innerHTML = info[3]; // 내용 데이터를 .dateBox 마크업 사이에 대입
  document.querySelector('.viewBox').innerHTML = info[4]; // 내용 데이터를 .view 마크업 사이에 대입
...
function 삭제함수(index) {
  ...
  console.log('삭제함수 실행');
  console.log(index);
  // 1. 배열내 특정한 인덱스 의 요소 제거, 배열변수명.splice( 삭제할인덱스, 개수 )
