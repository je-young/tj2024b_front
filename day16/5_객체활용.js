...
function 평가등록출력(클릭된사원번호) {
  ...
  let html = `<h3> 사원 평가 등록 </h3>
                <form>
                    ...
                    상반기평가 : <input class="상반기평가" type="text" />
                    하반기평가 : <input class="하반기평가" type="text"/> 
                    <button onclick="점수등록()" type="button"> 점수 등록 </button>
                </form>`;
  // 3, 출력
  div.innerHTML = html;
} // f end
...
// [4] 평가등록 > 점수등록 함수
function 점수등록(클릭된사원번호) {
  let year = document.querySelector('.평가연도').value;
  let firstScore = document.querySelector('.상반기평가').value;
  let secondScore = document.querySelector('.하반기평가').value;
