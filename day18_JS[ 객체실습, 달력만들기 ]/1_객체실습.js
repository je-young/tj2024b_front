...
// + 달력의 일 일정/내용
let contentArray = [
  { cno: 1, content: '학원개강', date: '2024-12-04', color: 'red' },
...
function calPrint() {
  ...
  for (let day = 1; day <= endDay; day++) {
    ...
    let planHtml = ``; //for 밖에 만든 이유 : 동일한 날짜의 2개 이상의 일정이 있을수 있으므로 누적
    for (let index = 0; index <= contentArray.length - 1; index++) {
      let plan = contentArray[index];
      if (plan.date == date3) {
        // 만약에 index 번째의 일정객체내 일정이 현재 보고있는 날짜와 같으면
        planHtml += `<div style="color : ${plan.color}"> ${plan.content}</div>`;
      }
    }
    // 일 출력
