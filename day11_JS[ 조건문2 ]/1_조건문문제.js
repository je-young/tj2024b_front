...
console.log("[4] 결과 : " + 정수1, 정수2, 정수3 + " 입니다.");
*/
...
if (점수 >= 90) {
  console.log("[5] 결과 : A등급");
} else if (점수 >= 80) {
  console.log("[5] 결과 : B등급 ");
} else if (점수 >= 70) {
  console.log("[5] 결과 : C등급 ");
} else {
  console.log("[5] 결과 : 재시험 ");
}
...
if (아이디 == "admin" && 비밀번호 == "1234") {
  console.log("[6] 결과 : 로그인 성공");
} else {
  console.log("[6] 결과 : 로그인 실패");
}
*/
...
// .indexOf('찾을값') : 지정한 찾을값을 배열내 존재하면 찾은 인덱스 반환 없으면 -1 반환 함수
// .push( 새로운값 ) : 지정한 새로운값을 배열내 마지막 인덱스 요소의 추가 함수

// 만약에 만약 존재하지 않으면(존재여부가 -1)
if (존재여부 === -1) {
  fruitList.push(과일명); // 배열에 새로운 과일 추가
  console.log(`[7] ${과일명} 이(가) 추가되었습니다.`);
} else {
  // 존재하면
  console.log("[7] 이미 존재하는 과일입니다.");
}
console.log(fruitList); // 현재 배열 출력
*/
...
/*
    ...
    [ 1. 경우의 수 ]
      p1 승리 경우 : p1=가위/p2=보 , p1=바위/p2=가위 , p1=보/p2=바위 <총: 3개>
      p2 승리 경우 : p2=가위/p1=보 , p2=바위/p1=가위 , p2=보/p1=바위 <총: 3개>
      무승부 경우 : p1=가위/p2=가위 , p1=바위/p2=바위, p1=보/p2=보 <총: 3개>
      ---> 총 9개 , 비교 연산 9번
      p1 승리 : ( p1 == 0 && p2 == 2 ) || ( p1 == 1 && p2 == 0 ) || ( p1 ==2 && p2 == 1 )
      p2 승리 : ( p2 == 0 && p1 == 2 ) || ( p2 == 1 && p1 == 0 ) || ( p2 ==2 && p1 == 1 )
      무승부 : ( p1 == 0 && p2 == 0 ) || ( p1 == 1 && p2 == 0 ) || ( p1 ==2 && p2 == 2 )
...
if (sheetList[예약좌석번호 - 1] == "O") {
  // -1 를 하는 이유 ? 배열 길이(데이터수)는 1부터~3 , 인덱스는 0~2
...
if (찾은인덱스 == -1) {
  // -1 이면 검색된 차량번호가 없다.
  console.log("입력한 차량번호는 주차 내역이 없습니다."); //메세지
} else {
  // -1 이 아니면 차량번호가 존재한다.
  console.log(`입력한 차량번흐의 추차 위치 : ${locationArray[찾은인덱스]}`);
}
*/
