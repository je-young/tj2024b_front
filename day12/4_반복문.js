...
for (let i = 1; i <= 5; i++) {
  // i는 1부터 5이하까지 1씩증가 반복 한다.
  // 만약에 i가 3이면 continue
  if (i == 3) {
    continue;
  } // 코드흐름이 continue 만나면 가장 가까운 증감식이동.
...
for (let i = 1; i <= 10; i++) {
  // i는 1부터 10이하까지 1씩증가 반복한다.
  if (i == 5) {
    break;
  } // 코드흐름이 break 만나면 가장 가까운 반복문 종료
...
for (;;) {
  // 무한루프 start  // 무한루프 종료조건 : 배열내 요소가 6개 이면 종료
  // (1) 무한입력
