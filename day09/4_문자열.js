/*

    문자열 처리 방법
        1.
    키워드( 문자 인데 기능이 있는 문자들 )
        'let'
        1. ' '
        2. " "
        3. ` ` ---- 키보드 왼쪽 TAB 키 위에 ~키

*/

// console.log(유재석) // 유재석 is not defined

// console.log ==> 문자( 미리 정의된/의미/기능이 포함된 문자 )
// 유재석 ==> 문자( 자바스크립트 회사에서 유재석 미리 정의X )

// [1] 문자열 처리
console.log('유재석1')
console.log("유재석2")
console.log(`유재석3`)

// [2] '작은따오표, 큰따오표 출력
console.log("유재석'반장'")
console.log('유재석"반장"')
console.log(`유재석'반장'`)

// [3] 이스케이프 / 제어문자 , 키보드 엔터 위에 원화기호(백슬래시)\
/*
    1. \n : console의 한줄 내림(줄바꿈)
    2. \" : "한개 출력 , \"\"
    3. \' : '한개 출력 , \'\'
    4. \\ : \한개 출력 , \\\\
    5. \t : 탭(들여쓰기)
*/
console.log('유재석\n \'반장\'')
console.log(`유재석\n강호동`)
console.log(`유재석\"강호동`)
console.log(`유재석\\강호동`)
console.log(`유재석\t강호동`)

// [4] 문자열 연결, 문자열 ( `백틱 )템플릿
console.log(`유재석` + `강호동`) // `문자A`+ `문자B` =>  `문자A문자B`
console.log(`유재석` + 40) // '문자A' + 숫자 => '문자A숫자'
console.log(`40` + `100`) //'숫자1 + 숫자2' => 40100

let age = 40; // '문자A' + 변수명 => 유재석40
console.log(`유재석` + age)
console.log(`유재석${age}강호동`) // `문자A${js코드}문자B`=> 유재석40강호동
console.log(`평균 : ${100 + 50}`) // 평균 : 150
// 방법1 : `문자그자체 문자그자체 문자그자체 ${js코드} 문자그자체`
// 방법2 : '문자그자체 문자그자체 문자그자체' + js코드 + '문자그자체'
// js 학습 목적 : HTML를 조작/제어/이벤트 하기 위해서 사용
// HTML은 변수X 연상X , 배열X, 타입X --> js 에서 html 작성시 js 코드와 합쳐서 작성할 수 있다. 
let 직원명1 = '유재석'
let 직원명2 = '강호동'
let nameHTML = `<ol> <li> ${직원명1} </li> <li> ${직원명2} </li> </ol>`
document.write(nameHTML) // document.write() : 현재 HTML에 출력하는 함수

// 예1 : 입력받은 데이터를 html 출력하시오.
// prompt('알람메세지) , 반환값 : 입력받은 값 1개
let 결과2 = prompt('[알람내용] 데이터 입력해주세요.')
// prompt 함수로부터 입력받은 값을 아래 코드에서 재사용 하고 싶다.
console.log(결과2)
let inpuitHTML = `<h3> 입력받은 데이터 : ${결과2} </h3>`
document.write(inpuitHTML)

// ----- 예]
let 결과1 = 10 + 5
// 10 + 5 한 결과의 15를 아래 코드에서 재사용( 연산 결과 저장하는 변수 ) 하고싶다.
console.log(결과1)

// 예2 : 2명의 사원이름을 입력받아서 아래와 같이 HTML(table)형식으로 출력하시오.
// 이유 : html 은 연산X , 저장X , 등등 없으므로 텍스트 그자체
// 새로운 값/데이터를 입력받아서 연산처리 가공(js) 하기위해서 js 사용한다.
// 순서도(알고리즘), 어떠한 작업의 순서/절차
// html 과 css 는 절차X , js코드는 위에서 아래로 실행
// 1. 1명의 사원이름을 입력받는다.                                  | prompt(), 반환값 : 입력받은 값
// 2. 입력받은 사원이름을 변수에 저장한다.                          | let 사원1 = prompt()
// 3. 1명의 사원이름을 입력받는다.                                  | prompt(), 반환값 : 입력받은 값
// 4. 입력받은 사원이름을 변수에 저장한다.                          | let 사원2 = prompt()
// 5. html형식의 문자로 출력할 변수와 구성한다.                     | tableHTML = `<table> ~~ </thble>`
// 6. 구성된 html형식의 문자를 html문서에 출력한다.                 | document.write(), 반환값 : X
// - 변수를 사용하는 이유 : 다음 코드에서 사용(출력 목적) 할려고.
let 사원1 = prompt(`사원명1 입력해주세요`); // [1][2] prompt() 함수를 사용하여 입력받은 값을 변수에 대입한다.
let 사원2 = prompt(`사원명2 입력해주세요`); // [3][4] prompt() 함수를 사용하여 입력받은 값을 변수에 대입한다.
let 출력할내용 = `<table>
                        <tr>
                            <th>번호</th>
                            <th>사원명</th>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>${사원1}</td>
                        </tr>
                        <tr>
                            <td>2</td>
                            <td>${사원2}</td>
                        </tr>
                    </table>`
document.write(출력할내용)

// 조과제 : 엘리베이터 의 작동 순서도를 한글로 작성하기 (최소 20개 이상)
// - 기본 시나리오1 : 나를 대신 하는 로봇이 1층에서 7층까지 올라가는 과정 작성
// - 기본 시나리오2 : 엘린베이터 기준의 작동 원리 순서를 작성
/*
엘리베이터 앞으로 이동한다.
버튼 위치를 확인한다
몇층으로 갈지 생각한다
버튼을 누르기 위해서 팔을 든다
버튼을 누른다
위 버튼인지 아래 버튼인지 확인한다
문이 열린다
엘리베이터 안쪽으로 이동한다
사람이 지나가는지 확인한다( 사람이 있다면 문이 다시 열린다
         밖에 사람이 있는지 확인한다
         사람이 있으면 열림 버튼을 누른다)
문닫힘 버튼을 누른다
엘리베이터 바깥으로 이동한다.
문이 닫힌다
층 버튼 위치를 확인한다
층 버튼을 누른다
엘리베이터가 이동한다
기다린다(문이 열리면 몇층인지 확인한다
   내가 생각한 층이 아니면 기다린다)
엘리베이가 멈춘다
문이 열린다
밖으로 이동한다
내린다
*/
