/*
    ...
    - 타입
        - 각 프로그래맹 언어들이 데이터들을 분류 하는 것 ( 예] 분리수거 )
        - 패트병인지?? 종이류 인지?? 유리인지?? 병인지??
        - 문자인지?? "" , '' , ``
        - 불리언인지?? true , false
        - 배열인지?? 154 , 3.15 , 747
...
*/
// 개발자도구[크롬F12] 에서 데이터 출력할때 사용하는 함수 , console.log( )
...
console.log(pointArray[0]) // 0번 인덱스에 존재하는 데이터 가 반환 // 100
console.log(pointArray[1]) // 1번 인덱스에 존제하는 데이터 가 반환 // 50
console.log(pointArray[2]) // 2번 인덱스에 존제하는 데이터 가 반환 // 30
...
pointArray.splice(1, 0, '하하') // 1변 인덱스에 0개 삭제하고 '하하' 대입(중간삽입) 한다.
console.log(pointArray) // ['강호동', '하하', '서장훈']

// [7] 배열변수의 특정 데이터 찾기 함수
// 배열변수명.indexOf( 찾을데이터 ) : 데이터가 배열내 존재하면 인덱스 번호 반환, 없으면 -1 반환
